...
if (!/insert into public\.kids_ai_budget[\s\S]*?'global'[\s\S]*?'all'[\s\S]*?'month'[\s\S]*?15\.00/i.test(migration)) failures.push("the approved USD 15 monthly pilot budget is not persisted");
