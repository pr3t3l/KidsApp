...
if (!/create policy provider_connection_owner_all[\s\S]*private\.has_recent_mfa/i.test(migration)) failures.push("provider connections do not require owner MFA");
...
const usageTable = migration.match(/create table public\.ai_usage_event\s*\(([\s\S]*?)\n\);/i)?.[1] ?? "";
...
if (!/create or replace function private\.has_recent_mfa\(\)[\s\S]*jsonb_array_elements[\s\S]*item->>'method'\s*=\s*'totp'[\s\S]*interval '15 minutes'/i.test(migration)) failures.push("recent administrative MFA does not verify a fresh TOTP assertion");
if (!/create policy product_setting_owner_all[\s\S]*with check \(private\.has_platform_role\(array\['platform_owner'\]\) and private\.has_recent_mfa\(\)\)/i.test(migration)) failures.push("product settings do not require recent MFA for writes");
const releaseDefinitions = [...migration.matchAll(/create or replace function public\.release_activity_version[\s\S]*?\$\$;/gi)];
if (!releaseDefinitions.length || !/private\.has_mfa\(\)/i.test(releaseDefinitions.at(-1)[0])) failures.push("direct activity release does not require MFA");
