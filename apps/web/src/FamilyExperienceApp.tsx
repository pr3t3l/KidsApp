...
function FamilyHeader({ locale, onFeedback }: { locale: Locale; onFeedback: () => void }) {
  const t = familyProductCopy[locale];
  return <header className="family-header"><Brand locale={locale}/><div className="family-header-actions"><span className="pilot-badge">{t.privatePilot}</span><button onClick={onFeedback}>♡ <span>{t.feedback}</span></button><span className="family-avatar">A</span></div></header>;
}
