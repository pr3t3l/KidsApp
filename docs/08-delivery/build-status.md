...
| 3. AI operation registry and provider gateway | Implemented; credentials stored | Versioned per-operation routing, OpenRouter, direct OpenAI and direct Anthropic adapters, metadata normalization, fallback and rollback tests pass. Alfredo stored OpenRouter and OpenAI keys through the hosted write-only Vault flow; health checks and live evaluations remain open. |
...
- authenticated OpenRouter/OpenAI health checks, approved deployments/data-processing routes, rate cards and live reconciliation; the USD 15 global spend ceiling is already active;
