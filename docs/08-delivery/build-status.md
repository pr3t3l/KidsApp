...
| 4. Usage, costs, budgets and admin controls | Implemented and locally verified | Usage ledger, reported/estimated/reconciled costs, effective-dated rates, scoped budgets, secret-store boundary, route evaluation and bilingual owner UI are implemented. Live rate configuration, budget creation, routing and provider billing reconciliation remain open. |
...
- Data: 65 public `kids_*` RLS tables across 19 forward-only Kids migrations, plus 11 no-op history markers for the shared Declassified migration ledger, private `kids_*` helpers and server-only secret access.
...
- authenticated OpenRouter/OpenAI health checks, approved deployments/data-processing routes, rate cards, spend authorization and live reconciliation;
