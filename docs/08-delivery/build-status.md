...
| 3. AI operation registry and provider gateway | Implemented; two providers verified | Versioned per-operation routing, OpenRouter, direct OpenAI and direct Anthropic adapters, metadata normalization, fallback and rollback tests pass. Alfredo stored and successfully health-checked OpenRouter and OpenAI through the hosted write-only Vault flow; live model evaluations remain open. |
| 4. Usage, costs, budgets and admin controls | Implemented; global budget hosted | Usage ledger, reported/estimated/reconciled costs, effective-dated rates, scoped budgets, secret-store boundary, route evaluation and bilingual owner UI are implemented. The owner-approved USD 15 monthly pilot budget is persisted; live rate configuration, routing and provider billing reconciliation remain open. |
...
- Data: 65 public `kids_*` RLS tables across 20 forward-only Kids migrations, plus 11 no-op history markers for the shared Declassified migration ledger, private `kids_*` helpers and server-only secret access.
...
- approved deployments/data-processing routes, rate cards and live reconciliation; authenticated OpenRouter/OpenAI health checks pass and the USD 15 global spend ceiling is already active;
...
- MFA repair deployment evidence: web `dpl_CbuSobi3xRJqLGfu9Le2ykVrDX2v` and API `dpl_1EzvHufMW1cs7wv3Q4aSbatYSVCA` reached `READY` from commit `58bfb430ed1ea3f64a2d8b5971be2935601d8f29`. The final aliases returned HTTP 200 and no new API runtime errors were present; secret values are intentionally absent from repository evidence.
- One-code administrative-session repair `7a96e617b78e09fcdc9873965c64a6c0c5299f4b` deployed as web `dpl_4EwD5EFKHTAnA36SyRpafzhKFfQN` and API `dpl_H6raSFRPtdchkGMVLhxibk1ieToj`; both reached `READY`. Remote migration `20260911133821_kids_admin_mfa_session_policy` is applied. The final web and health aliases returned HTTP 200, the removed step-up endpoint returned 404, the unauthenticated connections route remained 401 and Vercel reported no new runtime errors. Authenticated mobile connection creation remains the owner acceptance check.
- The owner acceptance attempt at 13:59 UTC confirmed the one-code session behavior and exposed a separate database authorization defect: Vault stored the key, the metadata insert returned `403`, and rollback deleted the new secret. Migration `20260911140514_kids_private_rls_helper_usage` is applied. A transaction under an authenticated owner/AAL2 claim proved the private gates and insert, then rolled back; Alfredo's next mobile attempt successfully stored direct OpenAI and OpenRouter connections.
- Migration `20260911142038_kids_provider_base_url_guard` repaired the affected OpenAI destination metadata without reading or rotating its encrypted key. Provider origins are now bound consistently in the UI, API and database, and validation errors cannot echo the submitted secret. Live connection health checks are the next gate.
- Migration `20260911143651_kids_default_ai_spend_guard` persisted the approved USD 15 monthly ceiling. Production now fails closed without a recent passing provider check, an effective rate and that global budget; a rotated key must be checked again. Warm Vercel instances refresh this control-plane state from Supabase within one second. OpenRouter health validates the authenticated key endpoint, not its public model catalog. Hosted OpenAI and OpenRouter checks both passed on 2026-09-11. A single observed transient Supabase `409` during a parallel cold-start read is now retried only for idempotent reads; mutations remain single-attempt. Local validation currently passes 25 web tests, the production web build and 82 API tests.
