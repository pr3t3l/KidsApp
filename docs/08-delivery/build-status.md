...
- Migration `20260911143651_kids_default_ai_spend_guard` persisted the approved USD 15 monthly ceiling. Production now fails closed without a recent passing provider check, an effective rate and that global budget; a rotated key must be checked again. Warm Vercel instances refresh this control-plane state from Supabase within one second. Local validation currently passes 24 web tests, the production web build and 80 API tests.
