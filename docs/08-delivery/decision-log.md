...
| DEC-063 | Use four product roles: `platform_owner`, `editorial_specialist`, `support_operator` and `family_adult`; require MFA for administrators and recent reauthentication for sensitive actions. | Approved | Separates full control, professional review, constrained support and family authority while preserving least privilege and auditability. |
...
| DEC-071 | Track general authentication time separately from the latest TOTP assertion; privileged recent-MFA actions require a recent TOTP AMR timestamp, and a newer password event cannot refresh that window. | Approved | AAL2 alone does not prove that the second factor was recently presented for a high-impact action. |
