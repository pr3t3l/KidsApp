...
alter table public.activity_version drop constraint if exists activity_version_release_channel_v2_check;
alter table public.activity_version add constraint activity_version_release_channel_v3_check
  check (release_channel in ('unreleased', 'synthetic-demo', 'pilot', 'founder_internal', 'family_pilot', 'production'));
alter table public.activity_version alter column release_channel set default 'unreleased';

create table public.pilot_cohort (
...
create table public.pilot_cohort_family (
  cohort_id uuid not null references public.pilot_cohort(cohort_id) on delete cascade,
...
create table public.pilot_cohort_activity (
  cohort_id uuid not null references public.pilot_cohort(cohort_id) on delete cascade,
...
alter table public.pilot_run add column if not exists session_id uuid references public.activity_session(session_id) on delete set null;
create unique index if not exists pilot_run_session_unique on public.pilot_run(session_id) where session_id is not null;
alter table public.family_feedback add column if not exists session_id uuid references public.activity_session(session_id) on delete set null;

alter table public.pilot_cohort enable row level security;
alter table public.pilot_cohort_family enable row level security;
alter table public.pilot_cohort_activity enable row level security;

create policy pilot_cohort_owner_select on public.pilot_cohort for select to authenticated using (private.has_platform_role(array['platform_owner', 'support_operator']) or exists (select 1 from public.pilot_cohort_family pcf where pcf.cohort_id = pilot_cohort.cohort_id and private.is_family_member(pcf.family_id, (select auth.uid()))));
create policy pilot_cohort_family_visible on public.pilot_cohort_family for select to authenticated using (private.has_platform_role(array['platform_owner', 'support_operator']) or private.is_family_member(family_id, (select auth.uid())));
create policy pilot_cohort_activity_visible on public.pilot_cohort_activity for select to authenticated using (private.is_editorial_member() or exists (select 1 from public.pilot_cohort_family pcf where pcf.cohort_id = pilot_cohort_activity.cohort_id and private.is_family_member(pcf.family_id, (select auth.uid()))));

create policy pilot_activity_version_select on public.activity_version for select to authenticated using (
  status = 'family_pilot' and release_channel = 'family_pilot' and exists (
    select 1 from public.pilot_cohort_activity pca
    ...
    where pca.activity_version_id = activity_version.activity_version_id
      and pca.content_hash = activity_version.content_hash
      and private.is_family_member(pcf.family_id, (select auth.uid()))
      and (pc.starts_at is null or pc.starts_at <= now()) and (pc.ends_at is null or pc.ends_at > now())
  )
);
create policy pilot_locale_select on public.activity_locale_v2 for select to authenticated using (exists (select 1 from public.activity_version av where av.activity_version_id = activity_locale_v2.activity_version_id and av.status = 'family_pilot' and exists (select 1 from public.pilot_cohort_activity pca join public.pilot_cohort pc on pc.cohort_id = pca.cohort_id and pc.state = 'active' join public.pilot_cohort_family pcf on pcf.cohort_id = pc.cohort_id where pca.activity_version_id = av.activity_version_id and pca.content_hash = av.content_hash and private.is_family_member(pcf.family_id, (select auth.uid())))));
create policy pilot_block_select on public.activity_block_v2 for select to authenticated using (exists (select 1 from public.activity_version av where av.activity_version_id = activity_block_v2.activity_version_id and av.status = 'family_pilot' and exists (select 1 from public.pilot_cohort_activity pca join public.pilot_cohort pc on pc.cohort_id = pca.cohort_id and pc.state = 'active' join public.pilot_cohort_family pcf on pcf.cohort_id = pc.cohort_id where pca.activity_version_id = av.activity_version_id and pca.content_hash = av.content_hash and private.is_family_member(pcf.family_id, (select auth.uid())))));
create policy pilot_chunk_select on public.activity_chunk for select to authenticated using (exists (select 1 from public.activity_version av where av.activity_version_id = activity_chunk.activity_version_id and av.status = 'family_pilot' and exists (select 1 from public.pilot_cohort_activity pca join public.pilot_cohort pc on pc.cohort_id = pca.cohort_id and pc.state = 'active' join public.pilot_cohort_family pcf on pcf.cohort_id = pc.cohort_id where pca.activity_version_id = av.activity_version_id and pca.content_hash = av.content_hash and private.is_family_member(pcf.family_id, (select auth.uid())))));
create policy pilot_adaptation_select on public.activity_adaptation for select to authenticated using (exists (select 1 from public.activity_version av where av.activity_version_id = activity_adaptation.activity_version_id and av.status = 'family_pilot' and exists (select 1 from public.pilot_cohort_activity pca join public.pilot_cohort pc on pc.cohort_id = pca.cohort_id and pc.state = 'active' join public.pilot_cohort_family pcf on pcf.cohort_id = pc.cohort_id where pca.activity_version_id = av.activity_version_id and pca.content_hash = av.content_hash and private.is_family_member(pcf.family_id, (select auth.uid())))));

revoke all on public.pilot_cohort, public.pilot_cohort_family, public.pilot_cohort_activity from anon, authenticated;
grant select on public.pilot_cohort, public.pilot_cohort_family, public.pilot_cohort_activity to authenticated;
grant select, insert, update, delete on public.pilot_cohort, public.pilot_cohort_family, public.pilot_cohort_activity to service_role;
...
begin
  ...
  select * into v_version from public.activity_version av
  ...
  foreach v_learner_id in array p_learner_ids loop
    insert into public.session_participant(session_id, learner_id, planned_role_id, primary_skill_id) values (v_session_id, v_learner_id, 'participant', p_primary_skill_id);
  end loop;
...
as $$
declare
  v_session public.activity_session%rowtype;
...
begin
  ...
  if not found or v_session.status = 'completed' or not exists (select 1 from public.family_membership fm where fm.family_id = v_session.family_id and fm.user_id = p_user_id) then raise exception 'session unavailable'; end if;
  select * into v_context from public.experience_context ec where ec.context_id = v_session.experience_context_id for update;
  select * into v_version from public.activity_version av where av.activity_version_id = v_session.activity_version_id;
  ...
  if v_version.status = 'family_pilot' and v_version.release_channel = 'family_pilot' then
    select count(*), coalesce(array_agg(distinct l.age_band), '{}') into v_count, v_bands from public.session_participant sp join public.learner l on l.learner_id = sp.learner_id where sp.session_id = p_session_id;
    ...
    insert into public.pilot_run(session_id, activity_version_id, activity_hash, family_id, facilitator_kind, participant_count, age_bands, duration_minutes, duration_fit, useful, outcome, observation_codes, recorded_by)
...
drop function if exists public.server_record_family_feedback(uuid, uuid, boolean, text, text, text, text, text, text, text, text);
create function public.server_record_family_feedback(
...
begin
  ...
  if p_session_id is not null and not exists (select 1 from public.activity_session s where s.session_id = p_session_id and s.family_id = p_family_id) then raise exception 'session access denied'; end if;
  insert into public.family_feedback(feedback_id, family_id, actor_user_id, useful, category, encrypted_comment, redacted_comment, screen, activity_version_id, session_id, app_version, locale, browser_family, journey_state)
  ...
  if p_session_id is not null then update public.pilot_run set useful = p_useful where session_id = p_session_id and family_id = p_family_id; end if;
...
as $$
  with eligible as (
    ...
    from public.activity_chunk ac join eligible e using (activity_version_id)
    ...
    from public.activity_chunk ac join eligible e using (activity_version_id)
    where ac.locale = match_locale and ac.embedding is not null and 1 - (ac.embedding <=> query_embedding) >= 0.55
    limit 8
  ), fused as (
...
revoke all on function public.server_record_family_feedback(uuid, uuid, boolean, text, text, text, text, uuid, text, text, text, text) from public;
grant execute on function public.server_record_family_feedback(uuid, uuid, boolean, text, text, text, text, uuid, text, text, text, text) to service_role;
