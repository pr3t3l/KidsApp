...
create table public.kids_family_evaluation_access (
...
create table public.kids_family_invitation (
...
alter table public.kids_family_evaluation_access enable row level security;
create policy kids_family_evaluation_access_browser_denied
  on public.kids_family_evaluation_access for select to authenticated using (false);
revoke all on public.kids_family_evaluation_access from anon, authenticated;
grant select, insert, update, delete on public.kids_family_evaluation_access to service_role;

alter table public.kids_family_invitation enable row level security;
create policy kids_family_invitation_admin_select
  on public.kids_family_invitation for select to authenticated
  using (
    private.kids_has_mfa()
    and private.kids_has_platform_role(array['platform_owner', 'support_operator'])
  );
revoke all on public.kids_family_invitation from anon, authenticated;
grant select on public.kids_family_invitation to authenticated;
grant select, insert, update, delete on public.kids_family_invitation to service_role;

alter table public.kids_activity_locale_v2
  drop constraint if exists kids_activity_locale_v2_completeness_check;
alter table public.kids_activity_locale_v2
  add constraint kids_activity_locale_v2_completeness_check
  check (completeness in ('draft', 'complete', 'reviewed', 'synthetic'));

create or replace function private.kids_has_active_evaluation_access(p_family_id uuid, p_user_id uuid)
returns boolean
...
as $$
  select exists (
    select 1
    from public.kids_family_evaluation_access fea
    join public.kids_family_membership fm on fm.family_id = fea.family_id
    where fea.family_id = p_family_id
      and fm.user_id = p_user_id
      and fea.expires_at > now()
  );
$$;
revoke all on function private.kids_has_active_evaluation_access(uuid, uuid) from public;
grant execute on function private.kids_has_active_evaluation_access(uuid, uuid) to authenticated, service_role;

create or replace function private.kids_user_can_access_version(p_activity_version_id text)
returns boolean
language sql stable security definer
set search_path = ''
as $$
      ...
      and (
        ...
        or (
          ...
          and exists (
            ...
            join public.kids_family_membership fm on fm.family_id = pcf.family_id
        ...
        or (
          ...
          and exists (
            ...
            join public.kids_family_evaluation_access fea on fea.family_id = fm.family_id
...
$$;
revoke all on function private.kids_user_can_access_version(text) from public;
grant execute on function private.kids_user_can_access_version(text) to authenticated, service_role;

create or replace function private.kids_family_can_access_version(
  p_family_id uuid,
    ...
    where av.activity_version_id = p_activity_version_id
      ...
      and (
        ...
        or (
          ...
          and av.risk_level in ('A', 'B')
          and exists (
            select 1 from public.kids_family_evaluation_access fea
...
drop policy if exists kids_published_activity_select on public.kids_activity_version;
drop policy if exists kids_pilot_activity_version_select on public.kids_activity_version;
drop policy if exists kids_activity_version_editorial_select on public.kids_activity_version;
create policy kids_activity_version_authorized_select
  on public.kids_activity_version for select to authenticated
  using (private.kids_user_can_access_version(activity_version_id));

drop policy if exists kids_published_adaptation_select on public.kids_activity_adaptation;
drop policy if exists kids_pilot_adaptation_select on public.kids_activity_adaptation;
create policy kids_activity_adaptation_authorized_select
  on public.kids_activity_adaptation for select to authenticated
  using (private.kids_user_can_access_version(activity_version_id));

drop policy if exists kids_published_chunk_select on public.kids_activity_chunk;
drop policy if exists kids_pilot_chunk_select on public.kids_activity_chunk;
create policy kids_activity_chunk_authorized_select
  on public.kids_activity_chunk for select to authenticated
  using (private.kids_user_can_access_version(activity_version_id));

drop policy if exists kids_locale_published_or_editorial_select on public.kids_activity_locale_v2;
drop policy if exists kids_pilot_locale_select on public.kids_activity_locale_v2;
create policy kids_activity_locale_authorized_select
  on public.kids_activity_locale_v2 for select to authenticated
  using (private.kids_user_can_access_version(activity_version_id));

drop policy if exists kids_block_published_or_editorial_select on public.kids_activity_block_v2;
drop policy if exists kids_pilot_block_select on public.kids_activity_block_v2;
create policy kids_activity_block_authorized_select
  on public.kids_activity_block_v2 for select to authenticated
  using (private.kids_user_can_access_version(activity_version_id));

create or replace function public.kids_hybrid_search_activity_chunks(
  query_text text,
...
as $$
  with keyword as (
    ...
    from public.kids_activity_chunk ac
    where ac.activity_version_id = match_activity_version_id
      and ac.locale = match_locale
      and private.kids_user_can_access_version(ac.activity_version_id)
      and ac.fts @@ websearch_to_tsquery('simple', query_text)
    limit 8
  ), semantic as (
    select ac.chunk_id,
           row_number() over (order by ac.embedding OPERATOR(extensions.<=>) query_embedding) as rank
    from public.kids_activity_chunk ac
  ...
  from fused
  join public.kids_activity_chunk ac using (chunk_id)
...
create or replace function public.kids_server_create_evaluation_preview(
  p_user_id uuid,
  p_family_id uuid,
  p_activity_version_id text,
...
declare
  ...
  v_locale jsonb;
begin
  if not private.kids_has_active_evaluation_access(p_family_id, p_user_id) then
  ...
  if p_planned_activity_id is not null and not exists (
    select 1
    from public.kids_planned_activity pa
...
$$;
revoke all on function public.kids_server_create_evaluation_preview(uuid, uuid, text, text, text, jsonb, jsonb, uuid[], uuid) from public;
grant execute on function public.kids_server_create_evaluation_preview(uuid, uuid, text, text, text, jsonb, jsonb, uuid[], uuid) to service_role;

create or replace function public.kids_decide_evaluation_companion_proposal(
  p_proposal_id uuid,
  p_decision text,
  p_option_id text,
...
begin
  ...
  if p_decision <> 'confirm' or v_proposal.kind <> 'replacement' then
    return query
      select d.context_id
      from public.kids_decide_companion_proposal(
  ...
  if not private.kids_has_active_evaluation_access(v_context.family_id, auth.uid()) then
    raise exception 'evaluation access denied';
  end if;
...
$$;
revoke all on function public.kids_decide_evaluation_companion_proposal(uuid, text, text, text) from public;
grant execute on function public.kids_decide_evaluation_companion_proposal(uuid, text, text, text) to authenticated;
