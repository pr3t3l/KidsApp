...
alter type public.kids_experience_status add value if not exists 'interrupted';

alter table public.kids_activity_session
  add column if not exists experience_context_id uuid unique references public.kids_experience_context(context_id) on delete cascade,
...
begin
  ...
  from public.kids_family_membership fm
  ...
  insert into public.kids_family(display_name, state_code, legal_matrix_version, consented_at)
  values (trim(p_display_name), p_state_code, p_legal_matrix_version, now())
  returning family_id into v_family_id;
  insert into public.kids_family_membership(family_id, user_id, role) values (v_family_id, auth.uid(), 'owner');
  insert into public.kids_family_preference(family_id, locale, units, time_zone, daily_minutes, usual_participants, max_mess)
...
$$;
revoke all on function public.kids_setup_family_with_learners(text, text, text, text, text, text, integer, integer, text, jsonb) from public;
grant execute on function public.kids_setup_family_with_learners(text, text, text, text, text, text, integer, integer, text, jsonb) to authenticated;
...
begin
  ...
  if old.status <> 'published' and new.status = 'published' then
    ...
    if (select count(*) from public.kids_activity_locale_v2 al where al.activity_version_id = new.activity_version_id and al.locale in ('en-US', 'es-US') and al.completeness = 'reviewed') <> 2 then raise exception 'two reviewed locales required'; end if;
    if exists (select 1 from (values ('en-US'), ('es-US')) wanted(locale) where not exists (select 1 from public.kids_activity_block_v2 ab where ab.activity_version_id = new.activity_version_id and ab.locale = wanted.locale)) then raise exception 'compiled blocks required in both locales'; end if;
  end if;
  return new;
end;
$$;
drop trigger if exists kids_guard_published_activity_version on public.kids_activity_version;
create trigger kids_guard_published_activity_version before update on public.kids_activity_version for each row execute function private.kids_guard_published_activity_version();

create or replace function private.kids_guard_published_activity_child()
returns trigger
language plpgsql security definer
set search_path = ''
as $$
...
begin
  ...
  if exists (select 1 from public.kids_activity_version av where av.activity_version_id = v_activity_version_id and av.status = 'published') then
...
drop trigger if exists kids_guard_published_activity_locale on public.kids_activity_locale_v2;
create trigger kids_guard_published_activity_locale before insert or update or delete on public.kids_activity_locale_v2 for each row execute function private.kids_guard_published_activity_child();
drop trigger if exists kids_guard_published_activity_block on public.kids_activity_block_v2;
create trigger kids_guard_published_activity_block before insert or update or delete on public.kids_activity_block_v2 for each row execute function private.kids_guard_published_activity_child();
drop trigger if exists kids_guard_published_activity_adaptation on public.kids_activity_adaptation;
create trigger kids_guard_published_activity_adaptation before insert or update or delete on public.kids_activity_adaptation for each row execute function private.kids_guard_published_activity_child();
drop trigger if exists kids_guard_published_activity_chunk on public.kids_activity_chunk;
create trigger kids_guard_published_activity_chunk before insert or update or delete on public.kids_activity_chunk for each row execute function private.kids_guard_published_activity_child();

drop policy if exists kids_adult_gate_self_all on public.kids_adult_gate_session;
revoke all on public.kids_adult_gate_session from authenticated;
grant select, insert, update, delete on public.kids_adult_gate_session to service_role;

create or replace function public.kids_server_verify_adult_gate(p_gate_session_id uuid, p_user_id uuid, p_answer_hash text)
returns table(family_id uuid, state text, expires_at timestamptz)
language plpgsql security definer
set search_path = ''
as $$
declare v_gate public.kids_adult_gate_session%rowtype;
begin
  select * into v_gate from public.kids_adult_gate_session ag where ag.gate_session_id = p_gate_session_id for update;
  ...
  if v_gate.challenge_hash <> p_answer_hash then
    update public.kids_adult_gate_session set failed_attempts = least(failed_attempts + 1, 3), expires_at = case when failed_attempts + 1 >= 3 then now() else expires_at end where gate_session_id = p_gate_session_id;
  ...
  update public.kids_adult_gate_session set verified_at = now(), expires_at = now() + interval '15 minutes' where gate_session_id = p_gate_session_id returning kids_adult_gate_session.expires_at into v_gate.expires_at;
  return query select v_gate.family_id, 'verified'::text, v_gate.expires_at;
end;
$$;
revoke all on function public.kids_server_verify_adult_gate(uuid, uuid, text) from public;
grant execute on function public.kids_server_verify_adult_gate(uuid, uuid, text) to service_role;
...
begin
  ...
  select * into v_version from public.kids_activity_version av where av.activity_version_id = p_activity_version_id and av.status = 'published' and av.release_channel = 'production' and av.content_hash = p_activity_hash and av.risk_level <> 'D';
...
create or replace function public.kids_server_update_session_progress(p_user_id uuid, p_session_id uuid, p_block_id text, p_status text)
returns jsonb
language plpgsql security definer
set search_path = ''
as $$
declare v_session public.kids_activity_session%rowtype; v_context public.kids_experience_context%rowtype; v_progress_state text; v_session_status text;
begin
  ...
  if not found or not exists (select 1 from public.kids_family_membership fm where fm.family_id = v_session.family_id and fm.user_id = p_user_id) then raise exception 'session unavailable'; end if;
  ...
  insert into public.kids_session_step_progress(session_id, step_id, state, completed_at)
  ...
  update public.kids_activity_session set current_step_id = p_block_id, status = v_session_status::public.kids_experience_status, paused_at = case when v_session_status = 'paused' then now() else paused_at end, updated_at = now() where session_id = p_session_id;
  update public.kids_experience_context set current_block_id = p_block_id, status = v_session_status::public.kids_experience_status, updated_at = now() where context_id = v_session.experience_context_id;
...
$$;
revoke all on function public.kids_server_update_session_progress(uuid, uuid, text, text) from public;
grant execute on function public.kids_server_update_session_progress(uuid, uuid, text, text) to service_role;
...
set search_path = ''
as $$
declare v_session public.kids_activity_session%rowtype; v_context public.kids_experience_context%rowtype;
begin
...
create or replace function public.kids_server_record_family_feedback(
...
begin
  ...
  insert into public.kids_family_feedback(feedback_id, family_id, actor_user_id, useful, category, encrypted_comment, redacted_comment, screen, activity_version_id, app_version, locale, browser_family, journey_state)
...
$$;
revoke all on function public.kids_server_record_family_feedback(uuid, uuid, boolean, text, text, text, text, text, text, text, text) from public;
grant execute on function public.kids_server_record_family_feedback(uuid, uuid, boolean, text, text, text, text, text, text, text, text) to service_role;

create or replace function public.kids_server_create_privacy_request(
  p_user_id uuid, p_family_id uuid, p_request_type text, p_idempotency_key text
) returns jsonb
language plpgsql security definer
set search_path = ''
as $$
declare v_request public.kids_privacy_request%rowtype;
begin
  ...
  if not exists (select 1 from public.kids_family_membership fm where fm.family_id = p_family_id and fm.user_id = p_user_id and fm.role = 'owner') then raise exception 'family owner required'; end if;
  insert into public.kids_privacy_request(family_id, requested_by, request_type, status, idempotency_key)
...
$$;
revoke all on function public.kids_server_create_privacy_request(uuid, uuid, text, text) from public;
grant execute on function public.kids_server_create_privacy_request(uuid, uuid, text, text) to service_role;

revoke insert, update, delete on public.kids_activity_session, public.kids_session_participant, public.kids_session_step_progress, public.kids_session_closeout from authenticated;
grant select on public.kids_activity_session, public.kids_session_participant, public.kids_session_step_progress, public.kids_session_closeout to authenticated;
revoke insert, update, delete on public.kids_experience_context, public.kids_preference_signal from authenticated;
grant select on public.kids_experience_context, public.kids_preference_signal to authenticated;
revoke insert, update, delete on public.kids_family_feedback, public.kids_privacy_request from authenticated;
grant select on public.kids_family_feedback, public.kids_privacy_request to authenticated;

create or replace function public.kids_search_eligible_replacements(p_context_id uuid, p_locale text, p_limit integer default 3)
...
as $$
  ...
  join public.kids_activity_version av on av.status = 'published' and av.release_channel = 'production' and av.activity_version_id <> ec.activity_version_id and av.risk_level <> 'D'
  join public.kids_activity_locale_v2 al on al.activity_version_id = av.activity_version_id and al.locale = p_locale and al.completeness = 'reviewed'
  where ec.context_id = p_context_id
    ...
    and exists (select 1 from public.kids_activity_block_v2 ab where ab.activity_version_id = av.activity_version_id and ab.locale = p_locale)
  order by av.activity_version_id
  limit least(p_limit, 3);
$$;
revoke all on function public.kids_search_eligible_replacements(uuid, text, integer) from public;
grant execute on function public.kids_search_eligible_replacements(uuid, text, integer) to authenticated;

create or replace function public.kids_decide_companion_proposal(p_proposal_id uuid, p_decision text, p_option_id text, p_idempotency_key text)
...
begin
  if auth.uid() is null then raise exception 'authentication required'; end if;
  select * into v_proposal from public.kids_companion_proposal cp where cp.proposal_id = p_proposal_id and cp.created_by = auth.uid() for update;
  if not found or v_proposal.state <> 'pending' then raise exception 'proposal unavailable'; end if;
  select * into v_context from public.kids_experience_context ec where ec.context_id = v_proposal.context_id and private.kids_is_family_member(ec.family_id, auth.uid()) for update;
  ...
  insert into public.kids_proposal_decision(proposal_id, actor_user_id, idempotency_key, decision, option_id) values (p_proposal_id, auth.uid(), p_idempotency_key, p_decision, p_option_id) on conflict (actor_user_id, idempotency_key) do nothing;
  ...
  if p_decision = 'confirm' and v_proposal.kind = 'replacement' then
    select av.snapshot into v_snapshot from public.kids_activity_version av where av.activity_version_id = p_option_id and av.status = 'published' and av.release_channel = 'production';
    select al.locale_payload into v_locale from public.kids_activity_locale_v2 al where al.activity_version_id = p_option_id and al.locale = v_context.locale and al.completeness = 'reviewed';
    select jsonb_agg(jsonb_build_object('id', ab.block_id, 'kind', ab.kind, 'version', ab.block_version, 'required', ab.required, 'data', ab.data) order by ab.position)
      into v_blocks from public.kids_activity_block_v2 ab where ab.activity_version_id = p_option_id and ab.locale = v_context.locale;
    ...
    update public.kids_experience_context ec set activity_version_id = p_option_id, applied_adaptation_id = null, immutable_delivery_snapshot = v_snapshot, effective_snapshot = v_snapshot, current_block_id = v_blocks->0->>'id', updated_at = now() where ec.context_id = v_context.context_id;
  elsif p_decision = 'confirm' then
    select aa.localized_content->v_context.locale->'blocks' into v_blocks from public.kids_activity_adaptation aa where aa.adaptation_id = p_option_id and aa.activity_version_id = v_context.activity_version_id;
    if v_blocks is null then raise exception 'adaptation is not approved for current version'; end if;
    update public.kids_experience_context ec set applied_adaptation_id = p_option_id, effective_snapshot = jsonb_set(ec.effective_snapshot, '{blocks}', v_blocks), current_block_id = v_blocks->0->>'id', updated_at = now() where ec.context_id = v_context.context_id;
  end if;
  update public.kids_companion_proposal set state = case when p_decision = 'confirm' then 'confirmed'::public.kids_proposal_state else 'rejected'::public.kids_proposal_state end, chosen_option_id = p_option_id, decided_at = now() where proposal_id = p_proposal_id;
  if p_decision = 'confirm' and v_proposal.preference_reason is not null then
    insert into public.kids_preference_signal(family_id, category, direction, strength, explicit_family_constraint, source_proposal_id, created_by)
  ...
  insert into public.kids_ai_audit_event(family_id, actor_user_id, event_type, resource_id, structured_detail) values (v_context.family_id, auth.uid(), 'companion_proposal_decided', p_proposal_id::text, jsonb_build_object('decision', p_decision, 'optionId', p_option_id));
