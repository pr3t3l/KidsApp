...
create or replace function private.family_can_access_version(p_family_id uuid, p_activity_version_id text, p_content_hash text)
returns boolean
...
alter table public.family_plan drop constraint if exists family_plan_family_id_starts_on_key;
create unique index if not exists one_active_family_plan_per_day on public.family_plan(family_id, starts_on) where status = 'active';

create or replace function public.server_ensure_family_plan(
  p_user_id uuid,
  p_family_id uuid,
  p_locale text,
  p_items jsonb
...
begin
  ...
  if exists (
    ...
    where not private.family_can_access_version(p_family_id, item->>'activityVersionId', item->>'contentHash')
  ...
  select fp.plan_id into v_plan_id from public.family_plan fp
  ...
  if v_plan_id is not null and not exists (
    ...
    where pa.plan_id = v_plan_id and not private.family_can_access_version(p_family_id, pa.activity_version_id, pa.delivery_hash)
  ) then
    return jsonb_build_object(
      'planId', v_plan_id,
      'items', coalesce((select jsonb_agg(jsonb_build_object('plannedActivityId', pa.planned_activity_id, 'activityVersionId', pa.activity_version_id, 'scheduledOn', pa.scheduled_on, 'position', pa.position, 'state', pa.state) order by pa.position) from public.planned_activity pa where pa.plan_id = v_plan_id), '[]'::jsonb)
    );
  end if;

  if v_plan_id is not null then
    update public.family_plan set status = 'cancelled', updated_at = now() where plan_id = v_plan_id;
    v_plan_id := null;
  end if;

  insert into public.family_plan(family_id, starts_on, locale, status, rule_version, explanation_codes, created_by)
  ...
  loop
    ...
    insert into public.planned_activity(plan_id, activity_version_id, scheduled_on, position, delivery_hash, state)
    values (v_plan_id, v_item->>'activityVersionId', current_date + ((v_position - 1) * 2), v_position, v_item->>'contentHash', 'planned');
  end loop;

  return jsonb_build_object(
    'planId', v_plan_id,
    'items', (select jsonb_agg(jsonb_build_object('plannedActivityId', pa.planned_activity_id, 'activityVersionId', pa.activity_version_id, 'scheduledOn', pa.scheduled_on, 'position', pa.position, 'state', pa.state) order by pa.position) from public.planned_activity pa where pa.plan_id = v_plan_id)
  );
end;
$$;
revoke all on function public.server_ensure_family_plan(uuid, uuid, text, jsonb) from public;
grant execute on function public.server_ensure_family_plan(uuid, uuid, text, jsonb) to service_role;
...
begin
  ...
  if not private.family_can_access_version(p_family_id, p_activity_version_id, p_activity_hash) then raise exception 'released activity version unavailable'; end if;
  select * into v_version from public.activity_version av where av.activity_version_id = p_activity_version_id and av.content_hash = p_activity_hash;
  ...
  if p_planned_activity_id is not null and not exists (
    select 1 from public.planned_activity pa join public.family_plan fp on fp.plan_id = pa.plan_id
  ...
  insert into public.activity_session(session_id, experience_context_id, family_id, planned_activity_id, activity_version_id, activity_hash, locale, status, current_step_id, started_at, created_by)
  ...
  if p_planned_activity_id is not null then update public.planned_activity set state = 'started' where planned_activity_id = p_planned_activity_id; end if;
...
$$;
revoke all on function public.server_start_activity_session(uuid, uuid, uuid, text, text, text, jsonb, jsonb, uuid[], text, uuid) from public;
grant execute on function public.server_start_activity_session(uuid, uuid, uuid, text, text, text, jsonb, jsonb, uuid[], text, uuid) to service_role;
...
begin
  ...
  if v_session.planned_activity_id is not null then
    update public.planned_activity set state = case when p_outcome in ('worked', 'partly') then 'completed' else 'skipped' end where planned_activity_id = v_session.planned_activity_id;
  end if;
...
create table public.product_setting_version (
...
alter table public.product_setting_version enable row level security;
create policy product_setting_owner_all on public.product_setting_version for all to authenticated using (private.has_platform_role(array['platform_owner'])) with check (private.has_platform_role(array['platform_owner']));
revoke all on public.product_setting_version from anon, authenticated;
grant select, insert, update on public.product_setting_version to authenticated;
grant select, insert, update, delete on public.product_setting_version to service_role;

create or replace function public.server_put_product_setting(
  p_actor_id uuid,
  p_brand text,
  p_locales text[],
  p_time_zone text,
...
declare
  ...
  v_id uuid;
begin
  if not exists (select 1 from public.platform_role_assignment pra where pra.user_id = p_actor_id and pra.role = 'platform_owner' and pra.active) then raise exception 'platform owner required'; end if;
  ...
  select coalesce(max(psv.version), 0) + 1 into v_version from public.product_setting_version psv;
  update public.product_setting_version set active = false where active;
  insert into public.product_setting_version(version, brand, locales, time_zone, guardrails, active, created_by)
  ...
  insert into public.admin_audit_event(actor_user_id, actor_role, event_type, resource_type, resource_id, detail)
...
$$;
revoke all on function public.server_put_product_setting(uuid, text, text[], text, jsonb) from public;
grant execute on function public.server_put_product_setting(uuid, text, text[], text, jsonb) to service_role;
